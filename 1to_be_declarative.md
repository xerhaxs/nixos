To configer (Paths):
* [ ] .config/backintime
* [ ] .config/blender
* [ ] .config/Clementine
* [ ] .config/easyeffects 
* [ ] .config/Fritzing
* [ ] .config/GIMP
* [ ] .config/heroic
* [ ] .config/kate
* [ ] .config/kdeconnect
* [X] .config/keepassxc
* [ ] .config/kmymoney
* [ ] .config/legcord
* [ ] .config/libreoffice
* [ ] .config/Mullvad VPN
* [ ] .config/Mumble
* [ ] .config/obs-studio
* [ ] .config/Proton Mail
* [ ] .config/protonmail
* [ ] .config/qalculate
* [ ] .config/rclone
* [ ] .config/tenacity
* [ ] .config/wasistlos or .config/whatsapp-for-linux
* [ ] .config/xournalpp

To configer (Files):
* [ ] .config/dolphinrc
* [ ] .config/tenacity.cfg
* [ ] .config/spectaclerc
* [ ] .config/skanpagerc
* [ ] .config/okularrc
* [ ] .config/kilerc
* [ ] .config/kdenliverc
* [ ] .config/katerc
* [ ] .config/gwenviewrc

.local/state/skanpagerc


# link primlauncher to .local/share/PrismLauncher from /home/jf/.local/share/PrismLauncher


* [X] test keepass xc config with ssh agent for ssh keyfiles https://github.com/keepassxreboot/keepassxc/blob/develop/docs/topics/SSHAgent.adoc + test browser integration
* [X] Test kitty close confirmation
* [ ] Test legcord home manager discord options https://gist.github.com/Julienraptor01/3d3428809b497aa9779bcb8cfc7e0750 + Setup JSON Settings!! https://home-manager-options.extranix.com/?query=discord&release=master
* [ ] Test Easyeffects Setup / Setup Audio Config https://gist.github.com/jtrv/47542c8be6345951802eebcf9dc7da31 https://home-manager-options.extranix.com/?query=easyeffects&release=master
* [X] Test Mullvad VPN Settings https://github.com/mullvad/mullvadvpn-app/blob/main/desktop/packages/mullvad-vpn/src/main/gui-settings.ts https://home-manager-options.extranix.com/?query=mullvad&release=master
* [ ] Test Mullvad VPN on Server due to disables enableExcludeWrapper (test local network connection!)
* [X] Test Command: nix flake update --commit-lock-file
* [ ] Dangerzone for NixOS? https://dangerzone.rocks/#downloads
* [ ] update pihole version on server
* [ ] unnest nixos.system.user.defaultuser to regular official nixos option
* [ ] remove userEnvironment/config parts and put this to home manager config files
* [X] setup brave / chromium with https://www.chromium.org/administrators/policy-templates and https://www.chromium.org/administrators/configuring-other-preferences/
* [ ] Test Paperless NGX https://search.nixos.org/packages?channel=25.11&include_modular_service_options=1&include_nixos_options=1&query=paperless&show=services.paperless.enable