...

{
  ...
  config =
    lib.mkIf
      (config.nixos.theme.catppuccin.enable && config.nixos.theme.theme.colorscheme == "catppuccin")
      {
        catppuccin = {
        ...
        boot.loader.grub.theme = lib.mkIf config.boot.loader.grub.enable (
          catppuccin.grub
          + lib.strings.toLower "/src/catppuccin-${config.nixos.theme.catppuccin.flavor}-grub-theme"
        );
